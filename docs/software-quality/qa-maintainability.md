...
점검 항목
* `[코드]` 한 도메인이 다른 도메인의 내부 클래스(Repository, Entity)에 직접 접근하지 않는가
* `[설계]` 도메인 간 의존이 한 방향으로 흐르고 순환 참조가 없는가
* `[코드]` 도메인 사이를 오갈 때 내부 Entity 대신 공개 인터페이스나 DTO로 주고받는가
...
점검 항목
* `[코드]` Controller에 비즈니스 로직이 없는가
* `[코드]` 외부 의존성이 구현체가 아니라 추상화에 의존하는가
* `[코드]` 시간, 랜덤, 현재 사용자 같은 암묵적 의존이 주입 가능한가
* `[프로세스]` 테스트 실행 시간이 개발 흐름을 끊지 않는가
...
점검 항목
* `[코드]` 같은 로직이 여러 곳에 복사되어 있지 않은가
* `[코드]` 매직 넘버가 상수로 분리되어 있는가
* `[설계]` 정책 변경 시 고쳐야 할 파일 수를 예측할 수 있는가
...
점검 항목
* `[인프라]` 포맷터와 정적 분석이 빌드에 연결되어 있는가
* `[인프라]` 규칙 위반이 경고가 아니라 실패로 처리되는가
* `[프로세스]` 규칙 변경 시 기존 코드 일괄 적용 방침이 있는가
