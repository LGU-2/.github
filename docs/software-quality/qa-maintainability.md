...
점검 항목
* `[코드]` `MNT-1-01` 한 도메인이 다른 도메인의 내부 클래스(Repository, Entity)에 직접 접근하지 않는가
* `[설계]` `MNT-1-02` 도메인 간 의존이 한 방향으로 흐르고 순환 참조가 없는가
* `[코드]` `MNT-1-03` 도메인 사이를 오갈 때 내부 Entity 대신 공개 인터페이스나 DTO로 주고받는가
...
* `[프로세스]` `MNT-2-04` 테스트 실행 시간이 개발 흐름을 끊지 않는가
