...
점검 항목
* `[코드]` 외부 호출이 실패했을 때의 흐름이 구현되어 있는가
* `[코드]` 실패 시 이미 만들어진 데이터가 불일치 상태로 남지 않는가
* `[설계]` 부분 성공 상태를 표현할 수 있는가
* `[설계]` 결과를 모르는 상태(타임아웃)를 실패와 구분하는가
...
점검 항목
* `[코드]` 빈 입력, null, 0, 음수, 최대값을 처리하는가
* `[코드]` 나눗셈 앞에 분모가 0이 될 가능성을 확인했는가
* `[코드]` 페이지 크기, 조회 건수 등에 상한이 있는가
* `[코드]` 상한을 클라이언트가 아니라 서버가 강제하는가
...
점검 항목
* `[코드]` 금액 계산에 float, double을 쓰지 않는가
* `[설계]` 반올림 정책(자리수, 모드)을 명시했는가
* `[설계]` DB 컬럼 타입이 애플리케이션 타입과 정밀도가 맞는가
* `[코드]` 총액을 여러 항목에 배분할 때 배분액의 합이 총액과 일치하는가
...
점검 항목
* `[코드]` 조회처럼 보이는 메서드가 상태를 바꾸지 않는가
* `[코드]` 반환값이 없는 메서드의 성공 여부를 호출자가 알 수 있는가
...
점검 항목
* `[프로세스]` 요구사항과 테스트가 대응되어 추적 가능한가
* `[프로세스]` 릴리스 전에 예외 시나리오를 함께 검증하는가
* `[프로세스]` 운영 결함의 원인 유형을 집계하는가
