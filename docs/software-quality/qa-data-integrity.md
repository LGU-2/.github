...
점검 항목
* `[코드]` 갱신 손실 가능성이 있는 흐름에 잠금을 적용했는가
* `[설계]` 충돌 빈도에 맞는 잠금 방식을 골랐는가
* `[설계]` 잠금 획득 순서가 일정해 교착 상태(deadlock)를 유발하지 않는가
* `[코드]` 낙관적 잠금 실패 시의 처리(재시도 또는 안내)가 정의되어 있는가
...
점검 항목
* `[설계]` 유일성이 필요한 값에 UNIQUE 제약이 있는가
* `[설계]` 값 범위 규칙이 CHECK 제약으로도 표현되어 있는가
* `[설계]` 참조 무결성이 필요한 곳에 외래 키가 있는가
* `[설계]` NOT NULL이 필요한 컬럼에 지정되어 있는가
...
점검 항목
* `[설계]` 트랜잭션 범위가 비즈니스 단위와 일치하는가
* `[코드]` 트랜잭션 안에서 외부 API를 호출하지 않는가
* `[코드]` checked 예외에서 롤백이 되는가
* `[코드]` 트랜잭션이 필요 이상으로 길지 않은가
...
점검 항목
* `[설계]` 서비스 간 작업에 보상 트랜잭션이 정의되어 있는가
* `[설계]` 이벤트 발행과 DB 커밋이 원자적인가
* `[코드]` 소비자가 중복 수신에 대비해 멱등한가
* `[인프라]` 처리 실패한 메시지가 유실되지 않고 격리되는가 (DLQ)
...
점검 항목
* `[프로세스]` 주기적 정합성 검사가 실행되는가
* `[인프라]` 어긋남을 발견했을 때 알림이 가는가
* `[프로세스]` 정정 절차가 기록으로 남는가
* `[설계]` 검사 대상과 허용 오차가 정의되어 있는가
