...
* `[설계]` `DI-2-03` 잠금 획득 순서가 일정해 교착 상태(deadlock)를 유발하지 않는가
...
점검 항목
* `[설계]` `DI-4-01` 트랜잭션 범위가 비즈니스 단위와 일치하는가
* `[코드]` `DI-4-02` 트랜잭션 안에서 외부 API를 호출하지 않는가
* `[코드]` `DI-4-03` checked 예외에서 롤백이 되는가
* `[코드]` `DI-4-04` 트랜잭션이 필요 이상으로 길지 않은가
...
* `[설계]` `DI-6-02` 이벤트 발행과 DB 커밋이 원자적인가
