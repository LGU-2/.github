...
점검 항목
* `[설계]` 기능별 지연시간 목표가 백분위수로 정의되어 있는가
* `[설계]` 목표 처리량과 그때의 자원 사용률을 함께 정했는가
* `[프로세스]` 목표를 부하 테스트로 검증했는가
...
점검 항목
* `[코드]` 컬렉션 조회 후 루프에서 연관 엔티티에 접근하지 않는가 (N+1)
* `[코드]` 목록 조회에서 건별 집계 쿼리를 반복하지 않는가
* `[설계]` 조회 조건에 맞는 인덱스가 있고 실제로 타는가
* `[프로세스]` 슬로우 쿼리를 정기적으로 확인하는가
...
점검 항목
* `[코드]` 반복문 안에서 반복마다 같은 연산을 다시 하지 않는가
* `[코드]` 조회 패턴에 맞는 자료구조를 쓰는가
* `[코드]` 대용량 처리에서 전체를 메모리에 올리지 않는가
* `[인프라]` 힙 크기와 GC 설정이 데이터 규모에 맞는가
...
점검 항목
* `[프로세스]` 캐시 도입 전에 쿼리와 인덱스 개선을 먼저 시도했는가
* `[설계]` 무효화 시점이 명확한가
* `[설계]` 캐시 미스 폭주(cache stampede)에 대비했는가
* `[설계]` 캐시가 죽어도 서비스가 동작하는가
...
점검 항목
* `[프로세스]` 주요 릴리스 전에 부하 테스트를 수행하는가
* `[프로세스]` 운영과 유사한 데이터 규모에서 검증하는가
* `[프로세스]` 한계점(포화 지점)을 알고 있는가
