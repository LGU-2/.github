...
점검 항목
* `[코드]` 싱글톤 빈에 가변 인스턴스 필드를 두지 않았는가
* `[코드]` 상태가 필요하면 지역 변수나 파라미터로 처리하는가
* `[인프라]` 세션과 캐시를 인메모리가 아니라 외부 저장소에 두는가
* `[인프라]` 업로드 파일을 로컬 디스크가 아니라 공유 스토리지에 두는가
...
점검 항목
* `[인프라]` 스케줄러가 다중 인스턴스에서 중복 실행되지 않도록 막았는가
* `[코드]` 인메모리 락으로 동시성을 제어하고 있지 않은가
* `[설계]` 인메모리 캐시의 무효화가 전 인스턴스에 전파되는가
...
점검 항목
* `[인프라]` 환경별 차이가 코드가 아니라 설정으로 분리되어 있는가
* `[인프라]` 설정과 비밀정보가 소스에 하드코딩되어 있지 않은가
* `[설계]` 외부 의존이 추상화 뒤에 있어 교체 가능한가
...
점검 항목
* `[설계]` 컬럼 제거를 4단계로 나눴는가
* `[인프라]` 대용량 테이블 DDL이 온라인으로 수행 가능한지 확인했는가
* `[프로세스]` 스키마 변경 후에도 이전 버전 코드로 롤백 가능한가
