...
점검 항목
* `[인프라]` 구조화된 형식(JSON)으로 남는가
* `[코드]` 요청 단위 상관관계 ID가 모든 로그에 붙는가
* `[코드]` 로그 레벨이 의미에 맞게 쓰이는가
* `[코드]` 개인정보와 인증 토큰이 남지 않는가
* `[설계]` 로그가 성능과 비용에 영향을 줄 만큼 과도하지 않은가
* `[인프라]` 보존 기간과 접근 권한이 정의되어 있는가
...
점검 항목
* `[인프라]` RED 또는 USE 관점의 기본 지표를 노출하는가
* `[코드]` 기술 지표뿐 아니라 비즈니스 지표(핵심 요청 처리 건수, 외부 연동 실패율)를 함께 수집하는가
* `[코드]` 태그(label) 카디널리티가 폭발하지 않는가
* `[인프라]` 수집 주기와 보존 기간이 정의되어 있는가
...
점검 항목
* `[인프라]` 서비스 간 호출에 추적 컨텍스트가 전파되는가
* `[인프라]` DB 쿼리와 외부 호출이 스팬으로 기록되는가
* `[설계]` 샘플링 정책이 정의되어 있는가
* `[코드]` 비동기 실행에서 추적 컨텍스트가 끊기지 않는가
...
점검 항목
* `[설계]` 주요 기능에 SLI와 SLO가 숫자로 정의되어 있는가
* `[인프라]` 알림이 원인이 아니라 증상(사용자 영향) 기준인가
* `[프로세스]` 알림에 대응 절차가 연결되어 있는가
* `[프로세스]` 무시되는 알림이 쌓이고 있지 않은가
...
점검 항목
* `[코드]` 예외에 원인 추적에 필요한 식별자가 포함되는가
* `[코드]` 외부 시스템 응답 코드가 원문 그대로 기록되는가
* `[인프라]` 배포 버전이 로그와 메트릭에 태그로 남는가
