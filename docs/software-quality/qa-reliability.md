...
점검 항목
* `[설계]` 기능 등급별로 가용성 목표를 다르게 정했는가
* `[설계]` 목표가 백분위수와 기간을 포함한 문장으로 적혀 있는가
* `[프로세스]` 목표 대비 실적을 정기적으로 확인하는 자리가 있는가
...
점검 항목
* `[코드]` 모든 외부 호출에 연결 타임아웃과 읽기 타임아웃이 설정되어 있는가
* `[설계]` 타임아웃 값이 상위 호출자의 타임아웃보다 짧은가 (타임아웃 예산 배분)
* `[인프라]` DB 커넥션 획득과 쿼리 실행에도 타임아웃이 있는가
...
점검 항목
* `[설계]` 재시도 대상 연산이 멱등한가
* `[설계]` 재시도 가능한 오류와 불가능한 오류를 구분했는가
* `[코드]` 재시도 간격에 지수 백오프와 지터를 적용했는가
* `[코드]` 재시도 횟수에 상한이 있는가
* `[설계]` 재시도 총 소요 시간이 상위 타임아웃 예산 안에 들어오는가
...
점검 항목
* `[인프라]` 실패율이 임계치를 넘으면 호출을 끊고 빠르게 실패하는가
* `[인프라]` 외부 연동별로 스레드풀 또는 커넥션풀이 분리되어 있는가
* `[설계]` 차단 상태에서의 대체 동작(fallback)이 정의되어 있는가
* `[설계]` 대체 동작이 안전한지 검토했는가
...
점검 항목
* `[인프라]` 과부하 시 전체가 느려지는 대신 일부를 거절하는가 (load shedding)
* `[인프라]` 큐 길이와 대기 시간에 상한이 있는가
* `[설계]` 우선순위가 낮은 기능을 먼저 끌 수 있는가 (graceful degradation)
* `[프로세스]` 기능을 끄는 스위치를 배포 없이 조작할 수 있는가
...
점검 항목
* `[설계]` 트랜잭션 경계가 복구 단위와 일치하는가
* `[코드]` checked 예외에서 롤백이 동작하는가
* `[프로세스]` 백업뿐 아니라 복원을 실제로 리허설했는가
* `[설계]` RPO와 RTO를 숫자로 정의했는가
* `[인프라]` 백업이 원본과 다른 장애 도메인에 저장되는가
...
점검 항목
* `[인프라]` 헬스체크가 의존 컴포넌트 상태를 반영하는가
* `[인프라]` 기동 완료 전에 트래픽을 받지 않는가 (readiness와 liveness 분리)
* `[인프라]` 종료 시 처리 중인 요청을 마치고 내려가는가 (graceful shutdown)
* `[설계]` liveness가 외부 의존성 상태에 좌우되지 않는가
