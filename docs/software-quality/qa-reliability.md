...
* `[설계]` `REL-2-02` 타임아웃 값이 상위 호출자의 타임아웃보다 짧은가 (타임아웃 예산 배분)
...
* `[설계]` `REL-5-04` liveness가 외부 의존성 상태에 좌우되지 않는가
