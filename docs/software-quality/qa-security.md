...
점검 항목
* `[코드]` 리소스 접근 시 소유권 또는 권한을 검증하는가
* `[코드]` 검증을 클라이언트가 보낸 식별자가 아니라 인증 주체 기준으로 하는가
* `[코드]` 목록 조회에서도 소유자 조건이 쿼리에 포함되는가
* `[설계]` 권한 모델이 문서화되어 있고 기본값이 거부인가
...
점검 항목
* `[코드]` 쿼리를 문자열 연결로 조립하지 않는가
* `[코드]` 정렬 컬럼, 테이블명 등 바인딩 불가 요소를 화이트리스트로 검증하는가
* `[코드]` 외부 명령 실행에 사용자 입력을 직접 넣지 않는가
...
점검 항목
* `[코드]` 모든 외부 입력을 서버에서 검증하는가
* `[코드]` 블랙리스트가 아니라 화이트리스트 방식인가
* `[코드]` 길이와 범위 상한이 있는가
* `[코드]` 파일 업로드에서 확장자, MIME 타입, 크기, 저장 경로를 검증하는가
* `[인프라]` 요청 본문 크기 상한이 설정되어 있는가
...
점검 항목
* `[코드]` 비밀번호를 단방향 해시로 저장하는가 (BCrypt, Argon2 등)
* `[코드]` 로그와 예외 메시지에 개인정보, 인증 토큰, 쿼리 원문이 남지 않는가
* `[코드]` 응답 DTO가 엔티티를 그대로 노출하지 않는가
* `[설계]` 저장 시 암호화가 필요한 항목을 등급으로 구분했는가
...
점검 항목
* `[설계]` 상태 변경 이력에 변경 주체가 기록되는가
* `[설계]` 관리자 행위에 감사 로그가 남는가
* `[인프라]` 이력이 애플리케이션에서 삭제되지 않도록 보호되는가
* `[프로세스]` 감사 로그를 정기적으로 검토하는가
...
점검 항목
* `[인프라]` 로그인, 인증번호 발송 등에 시도 횟수 제한이 있는가
* `[인프라]` IP 또는 계정 단위 레이트 리밋이 있는가
* `[설계]` 대량 조회 API에 상한과 인증이 걸려 있는가
* `[코드]` 인증 실패 응답이 계정 존재 여부를 구분해서 알려 주지 않는가
